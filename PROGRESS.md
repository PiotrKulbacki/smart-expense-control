...
- **Dashboard** — panel „Ostatnie transakcje" z przewijaniem (`ScrollArea`, `max-h-[350px]`, stylizowany scrollbar).
...
2. **Migracja DB (nowa):** `20260714120000_add_transaction_receipt_group_id` — kolumna `receiptGroupId` na `transactions`; wdroży się przy `migrate:deploy` na Vercel.
3. **Vercel env:** `STRIPE_PRO_PRICE_PLN|EUR|GBP`, `NEXT_PUBLIC_SENTRY_DSN` (opcjonalnie org/project/token).
...
- `RecentTransactionsList` — scroll `max-h-[350px]` na pulpicie; płynne rozwijanie grup splitów w Historii.
